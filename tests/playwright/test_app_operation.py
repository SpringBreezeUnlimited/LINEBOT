...
def main():
    ...
    if not http_check():
        print(
            "NOT_RUNNING: http://localhost:8080 が起動していません。SKILL.md の手順で起動してください。"
        )
        sys.exit(2)
    ...
    if not username or not password:
        print(
            "APP_USERNAME / APP_PASSWORD が .env に設定されていません。設定してください。"
        )
        sys.exit(3)
    ...
    with sync_playwright() as p:
        ...
        is_login_page = (
            ...
            or page.locator("text=Login").count() > 0
            or page.locator("text=ログイン").count() > 0
        )
        ...
        if is_login_page:
            ...
            try:
                ...
                logged_in = (
                    ("/home" in page.url)
                    or page.locator("text=Logout").count() > 0
                    or page.locator("text=ログアウト").count() > 0
                )
                if not logged_in:
            ...
            if (
                ...
                print(
                    "ログイン画面ではありませんが、ログイン状態を確認できませんでした。URL=",
                    page.url,
                )
